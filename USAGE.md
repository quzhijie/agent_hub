...
桌面看板顶栏「**+ 新建流水线**」把一件多步的活儿编成一条**线性流水线**——例如 `plan → implement → review`。每一步起一个**受控 agent**,自动把这步的 prompt 发进去;它干完就停下,**等你点「批准」才进下一步**。整条流水线跑在一个**专属 git worktree + 分支**里,完全不碰你当前工作区。

> **谁在指挥?没有 LLM 在指挥。** 编排器(`backend/app/orchestrator.py`)是个确定性状态机;它唯一往席位打字的口子带**硬编码白名单**:只能写本流水线自己建的、且被标为 `orchestrated` 的席位——**你手动建的交互式席位它永远碰不到**。功能默认开着,但**不建流水线就什么都不会跑**。
...
- **从大纲导入**:填一个本机 `OUTLINE.md` 路径 →「解析大纲」。按 `##` 标题 / 编号 / 勾选项**自动切成步骤**(纯规则,不用 LLM);整份大纲会被复制进 worktree 作 `OUTLINE.md`,每个 agent 都能看到全局、只做自己那步。

无论哪种来源,下面的**步骤列表都能改**:改名、换 provider(claude / codex …)、改 prompt、加 / 删步骤、`↑↓` 排序。确认后点「**创建并开始**」。

### 3. 跑起来:每步都是一道审批闸

流水线卡片按步显示进度,每步状态:`待开始 → 启动中 → 进行中 → 待批准 → 已完成`。

- 某步 agent 干完(屏幕从「工作中」落回静止),该步转 **待批准**,卡片会**贴出它最后的产出**预览;
- 想看真东西就点该步的「**跳到终端**」进去——这些就是普通 `hub-` 席位,tmux / handmux 里也在;
- 满意就点「**批准 → 下一步 ▶**」,下一步才启动;整条走完流水线转「**已完成**」。
- **每步 prompt 结尾会让 agent 单独打一行 `<<<PHASE-DONE>>>`** 当"我做完了"的人眼标记;但真正的完成判定看的是**席位由动转静**,不是去 grep 这行。

### 4. 中止 / 删除 / 取回成果

- 「**中止**」:结束这条流水线的所有 agent,但 **worktree 和分支都保留**,方便你去查看或合并成果。
- 「**🗑 删除**」:先中止(若在跑)、清掉它的 agent,并**移除 worktree 目录**(分支仍保留);流水线记录一并删掉。
